...
const Home = ({ navigation, route }) => {
  const { produk, token, userDetail } = useContext(RootContext);
  const [cartList, setCartList] = useState([]);

  const fetchCart = () =>
    cartAPI.getCartList(userDetail.id, token).then((res) => {
      const carts = res.data
        .map((cart) => {
          const prod = produk.find(({ id }) => id == cart.produk_id);
          if (prod) {
  ...
  return (
    <View style={styles.box}>
      <View style={[styles.inner, highlighted ? styles.innerOrange : {}]}>
        ...
        <TouchableOpacity
          onPress={() => {
            navigation.navigate("DetailProduk", { id });
          }}
        >
          <AntDesign
            name="rightcircleo"
            size={24}
            color={highlighted ? "white" : "#5D7283"}
          />
        </TouchableOpacity>
        <Text style={[styles.textbox, highlighted ? { color: "white" } : {}]}>
          {text}
        </Text>
      </View>
    </View>
